...
ngx_epoll_process_events(ngx_cycle_t *cycle, ngx_msec_t timer, ngx_uint_t flags)
{
    ...
    for (i = 0; i < events; i++) {
        /*
        *  data.ptr是初始化的时候设置的，参考 epoll_wait 
        *  而 epoll_wait 也是结束init阶段传进来的设置
        */
        /*
        *  epoll_event.data是用户传入的 attach 信息，每个连接都是一样的
        *  （java中selector也是可以传attach信息的）
        *  这中方式算是一种钩子数据结构，在异步模型中很有用。
        */
        ...
        /***** 读事件处理 *****/
        rev = c->read;
        ngx_log_error(NGX_LOG_ERR, cycle->log, 0, "yyy: process event........");
        ...
        if ((revents & EPOLLIN) && rev->active) {
            ...
            if (flags & NGX_POST_EVENTS) {
                ...
                // 调用handler , 例如 ngx_http_wait_request_handler
                rev->handler(rev); // handler 是初始化的时候指定的，参考 ngx_http_init_connection
            }
        }
        /***** 写事件处理 ******/
        wev = c->write;
