...
ngx_event_accept(ngx_event_t *ev)
{
    ...
    do {
        ...
        ngx_log_error(NGX_LOG_ERR, ev->log, 0,"yyy:: ngx_event_accept.....");
#if (NGX_HAVE_ACCEPT4)
        if (use_accept4) {
        ...
        */
        ...
        ngx_memcpy(c->sockaddr, sa, socklen);// 将sa内容拷贝到c->sockaddr中
...
#endif
        //将sockaddr字段保存的对端地址格式化为可读字符串，并保存在 addr_text 字段
        if (ls->addr_ntop) {
