...
ngx_event_process_init(ngx_cycle_t *cycle)
{
    ...
    /*
    *   遍历每个监听套接字（nginx支持同时监听多个套接字，注意虚拟主机配置里面，可以设置不同的监听端口）
    */
    ls = cycle->listening.elts;
    for (i = 0; i < cycle->listening.nelts; i++) {
        /*
        *  ngx_connection_t结构体用于描述一个网络连接
        *  每当服务器产生一个新的socket时，就申请一个ngx_connection_t结构体
        *  1) 保存socket信息
        *  2）设置read/write处理函数
        */
        /*
        *   监听端口也需要申请一个ngx_connection_t结构体
        */
        ...
        *   定义accept连接的处理函数
        */
        rev->handler = ngx_event_accept;  //读事件回调函数
        ...
        /*
        * epoll模型下
        *    ngx_add_event对应   ngx_epoll_add_event
        *    ngx_add_conn对应 ngx_epoll_add_connection
        */
