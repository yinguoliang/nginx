...
ngx_http_wait_request_handler(ngx_event_t *rev)
{
    ...
    c = rev->data;
    ngx_log_error(NGX_LOG_ERR, c->log, 0, "yyy::ngx_http_wait_request_handler....");
...
ngx_http_process_request_line(ngx_event_t *rev)
{
    ...
    c = rev->data;
    r = c->data;
    ngx_log_error(NGX_LOG_ERR, c->log, 0, "yyy::ngx_http_process_request_line......");
    ...
    for ( ;; ) {//为什么要循环？？？？？？？
        ...
        if (rc == NGX_AGAIN) {
            n = ngx_http_read_request_header(r); //这里应该是读取http报文的首行，里面含有Method/协议版本/uri等内容
        ...
        /*
        *  按自动机来解析http报文，解析出来的内容放到r结构中的对应字段里面
        */
        rc = ngx_http_parse_request_line(r, r->header_in);//解析读取到的内容，解析出method, version, uri等
        ngx_log_error(NGX_LOG_ERR, c->log, 0, "yyy::ngx_http_process_request_line loop ......\n");
        if (rc == NGX_OK) {
            ...
            * 调用 ngx_http_find_virtual_server 函数来查找用来处理该请求的虚拟服务器配置，
            * 之前通过端口和地址找到的默认配置不再使用，找到相应的配置之后，则直接调用ngx_http_process_request（）
            * 函数处理该请求，
            * 因为http0.9是最原始的http协议，它里面没有定义任何请求头，显然就不需要读取请求头的操作。
            */
...
ngx_http_process_request_headers(ngx_event_t *rev)
{
    ...
    for ( ;; ) {
        ...
        *  1）读取header
        *  2) 解析header
        *  3) 调用header处理函数（ 参考 ngx_http_headers_in[] ）
        */
        if (rc == NGX_AGAIN) {
        ...
        if (rc == NGX_OK) {
            ...
            /*
            *  调用header头处理函数，参考 ngx_http_headers_in[]
            *  例如 Host 头的处理函数为： ngx_http_process_host
            */
...
ngx_http_process_request(ngx_http_request_t *r)
{
    ...
    c = r->connection;
    ngx_log_error(NGX_LOG_ERR, c->log, 0, "yyy::ngx_http_process_request....\n");
#if (NGX_HTTP_SSL)
...
ngx_http_set_keepalive(ngx_http_request_t *r)
{
    ...
    c = r->connection;
    rev = c->read;
    ngx_log_error(NGX_LOG_ERR, c->log, 0, "yyy::ngx_http_set_keepalive......");
    clcf = ngx_http_get_module_loc_conf(r, ngx_http_core_module);
...
ngx_http_keepalive_handler(ngx_event_t *rev)
{
    ...
    c = rev->data;
    ngx_log_error(NGX_LOG_ERR, c->log, 0,"yyy::ngx_http_keepalive_handler....\n");
