...
{
    ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,"yyy::ngx_http_handler.....");
    ...
    r->gzip_vary = 0;
#endif
    ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,"yyy::start to run >>> ngx_http_core_run_phases().....");
...
ngx_http_core_run_phases(ngx_http_request_t *r)
{
    ...
    ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,"yyy::ngx_http_core_run_phases.....");
    ...
    ngx_http_phase_handler_t *ph)
{
    ...
    ngx_str_t  path;
    /*
    *  如果有content_handler，则只执行content_handler
    *  注意：每个location都可以有自己的content_handler
    *       如果有content_handler，就不再执行handler了
    */
    ...
    /*
    *  如果没有content_handler，就执行注册的handler
    */
    rc = ph->handler(r);
